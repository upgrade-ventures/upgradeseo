 ...
 *
 * The Agents SDK eagerly imports all three (~540 kB with the @ai-sdk/openai +
 * @ai-sdk/anthropic providers they pull in) solely to build its lazy default
 * provider in `resolveModel()` — which only runs when `getModel()` returns a
 * bare model-id string. OnboardingChatAgent returns a constructed OpenRouter
 * LanguageModel instead, so the default provider is dead code; this stub keeps
 * it out of the eager worker bundle. Same pattern as just-bash-stub.ts.
 */
const STUBBED_MESSAGE =
  ...
  "OnboardingChatAgent overrides getModel with OpenRouter";
